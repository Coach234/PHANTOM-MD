const {
  pnix,
  isPrivate,
  getBuffer,  // Make sure getBuffer is imported correctly
} = require("../lib/");
...
pnix(
  ...
  async (message, match, m) => {
    ...
    let randomName = getRandom(".mp3");
    let img = await getBuffer(anu.thumbnail);
    await message.client.sendMessage(message.jid, { text: `_Downloading...  ${titleYt}_`, contextInfo: { externalAdReply: { title: `${message.pushName}`, sourceUrl: anu.url, mediaUrl: anu.url, mediaType: 1, showAdAttribution: true, renderLargerThumbnail: true, thumbnailUrl: anu.thumbnail } } }, { quoted: m });
    const stream = ytdl(anu.url, {
      filter: (info) => info.audioBitrate == 160 || info.audioBitrate == 128,
    }).pipe(fs.createWriteStream(`./media/${randomName}`));
    await new Promise((resolve, reject) => {
